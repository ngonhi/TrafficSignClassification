...
print('Load model')
model = pickle.load(open('clf_mlp_hog_300.pkl', 'rb'))
...
def feature_extraction(image):
    (HOGfeature, hogImage) = feature.hog(image, orientations=9, pixels_per_cell=(4, 4),
...
def classify(file_path):
    ...
    image = transform.resize(image, (40, 40))
    print(image.shape)
    HOGfeature = feature_extraction(image).reshape((1, -1))
    print(HOGfeature.shape)
    pred = model.predict(HOGfeature)[0]
    print(pred)
    sign = classes[pred]
    print(sign)
